...
## make feature vs. weight
feat2weight = {}
for ef in feat2id.keys():
    fid = feat2id[ef]
    feat2weight[ef] = lr.coef_[0][fid]
# sort
tmp = []
for ef, ew in sorted(feat2weight.items(), key=lambda x: x[1]):
    tmp.append( (ef, ew) )
# 10 features with the highest weights
print('10 features with the highest weights')
for i in range(-1, -10, -1):
    print(' {} {}'.format(tmp[i][0], tmp[i][1]) )
# 10 features with the lowest weights
print('10 features with the lowest weights')
for i in range(10):
    print(' {} {}'.format(tmp[i][0], tmp[i][1]) )
