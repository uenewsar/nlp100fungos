...
print(' classes: {}'.format(lr.classes_))
'''

## evaluation
# evaluation data (which were chosen from training data)
eval_stc = [
    'mazel tov to a film about a family\'s joyous life acting on the yiddish stage .', # positive
    'standing in the shadows of motown is the best kind of documentary , one that makes a depleted yesterday feel very much like a brand-new tomorrow .', # positive
    'it\'s nice to see piscopo again after all these years , and chaykin and headly are priceless .', # positive
    'as it stands , crocodile hunter has the hurried , badly cobbled look of the 1959 godzilla , which combined scenes of a japanese monster flick with canned shots of raymond burr commenting on the monster\'s path of destruction .', # negative
    'the thing looks like a made-for-home-video quickie .', # negative
    'enigma is well-made , but it\'s just too dry and too placid .' # negative
]

for es in eval_stc:
    ed = Instance()
    ed.stc = normalize_stc(es)
    ed.words = ed.stc.split(' ')
    (ed.feat, ed.feat_vec) = create_feat(ed.words, feat2id)
    
    est_label = lr.predict([ed.feat_vec])[0]
    est_prob = lr.predict_proba([ed.feat_vec])[0][np.where(lr.classes_==est_label)][0]

    print('estimated_label={} probability={}'.format(est_label, est_prob))
